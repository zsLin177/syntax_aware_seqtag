        ...
        for i, batch in enumerate(bar, 1):
            ...
            if self.args.policy_grad:
                ...
                if self.aux_model is not None:
                    ...
                    skl, if_false_mask = self.aux_model.avg_metric(q, p=torch.cat((torch.zeros_like(labels[:,0]).unsqueeze(-1), labels), -1), threshold=self.args.threshold)
                    loss = self.model.loss(score, labels, mask, 1-skl) #skl是不确定性
                    ...
                    skl, if_false_mask = self.model.avg_metric(q, p=torch.cat((torch.zeros_like(labels[:,0]).unsqueeze(-1), labels), -1), threshold=self.args.threshold, req_grad=True)
